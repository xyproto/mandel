...
func estimateNormal(p vec3) vec3 {
	eps := 0.001
	return vec3{
		mandelbulb(p.add(vec3{eps, 0, 0})) - mandelbulb(p.sub(vec3{eps, 0, 0})),
		mandelbulb(p.add(vec3{0, eps, 0})) - mandelbulb(p.sub(vec3{0, eps, 0})),
		mandelbulb(p.add(vec3{0, 0, eps})) - mandelbulb(p.sub(vec3{0, 0, eps})),
	}.normalize()
}
...
func renderPixel(x, y int) color.Color {
	...
	for i := 0; i < maxIter; i++ {
		...
		if d < 0.001 {
			...
			diffuse := math.Max(0, normal.dot(lightDirection))
			specular := math.Pow(math.Max(0, normal.add(lightDirection).normalize().dot(direction.mul(-1))), phongShininess)
			ao := 1 - ambientOcclusion*math.Pow(t/100, 2)
			intensity := ao * (phongDiffuse*diffuse + phongSpecular*specular)
			return color.RGBA{uint8(255 * intensity), uint8(128 * intensity), uint8(255 * intensity), 255}
		}
		t += d
		if t > 100 {
			break
